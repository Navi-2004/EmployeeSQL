...
<table>
    <thead>
      <tr>
        ...
        <th>
  Salary
  <button className='but' onClick={() => sortSalary('asc')}>&uarr;</button>
  <button className='but' onClick={() => sortSalary('desc')}>&darr;</button>
</th>
        <th>Actions</th>
      </tr>
    </thead>
  ...
  <ul className="pagination">
    ...
    <li className={`page-item ${currentItems.length < itemsPerPage ? 'disabled' : ''}`}>
      <button className="page-link" onClick={() => paginate(currentPage + 1)} disabled={currentItems.length < itemsPerPage}>
        Next
      </button>
    </li>
  </ul>
